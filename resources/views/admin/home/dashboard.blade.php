...
@section('breadcrumb_stats')
    <div class="d-flex justify-content-center me-5">
        <div>
            <span class="d-block"><span class="label">PROCESSING</span></span>
            <span class="value">{{ $process_count ?? 0 }}</span>
        </div>
        <div class="ms-3 mt-2">
    ...
    <div class="d-flex justify-content-center">
        <div>
            <span class="d-block"><span class="label">COMPLETED</span></span>
            <span class="value">{{ $invoice_complete_count ?? 0 }}</span>
        </div>
        <div class="ms-3 mt-2">
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            ...
                            <div class="project-content">
                                <h6>Invoices</h6>
                                <ul>
                                    <li><strong>Processing</strong><span>{{ $process_count ?? 0 }}</span></li>
                                    <li><strong>Completed</strong><span>{{ $invoice_complete_count ?? 0 }}</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            ...
                            <div class="project-content">
                                <h6>Users</h6>
                                <ul>
                                    <li><strong>Resellers</strong><span>{{ $reseller_count ?? 0 }}</span></li>
                                    <li><strong>Customers</strong><span>{{ $user_count ?? 0 }}</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            ...
                            <div class="project-content">
                                <h6>Revenue</h6>
                                <ul>
                                    <li><strong>Sold</strong><span>${{ number_format($sold_ticket_sum ?? 0, 2) }}</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card overflow-hidden">
                    <div class="card-header bg-transparent pd-b-0 pd-t-20 bd-b-0">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-10">Ticket Sales Overview</h4>
                            <i class="mdi mdi-dots-horizontal text-gray"></i>
                        </div>
                        <p class="text-muted tx-12 mb-2">Overview of ticket sales and order processing activity.</p>
                    </div>
                    <div class="card-body pd-y-7">
                        <div class="area chart-legend mb-0">
                            <div><i class="mdi mdi-album text-primary me-2"></i> Total Revenue</div>
                            <div><i class="mdi mdi-album text-pink me-2"></i> Processing Orders</div>
                        </div>
            ...
            <div class="col-sm-12 col-md-12 col-lg-12 col-xl-4">
                <div class="card overflow-hidden">
                    <div class="card-body pb-3">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-10">Events &amp; Status</h4>
                            <i class="mdi mdi-dots-horizontal text-gray"></i>
                        </div>
                        <p class="text-muted tx-12 mb-3">Upcoming events and their current ticket status.</p>
                        <div class="table-responsive mb-0 projects-stat tx-14">
                            <table class="table table-hover table-bordered mb-0">
                                ...
                                <tbody>
                                    @forelse (($upcoming_events ?? collect())->take(5) as $event)
                                        @php
                                            $total = (int) ($event->total_ticket_count ?? 0);
                                            $sold = (int) ($event->sold_ticket_count ?? 0);
                                            $isSoldOut = $total > 0 && $sold >= $total;
                                        @endphp
                                        <tr>
                                            <td>
                                                <div class="project-names">
                                                    <p class="d-inline-block font-weight-semibold mb-0">{{ $event->event_name }}</p>
                                                </div>
                                            </td>
                                            <td>
                                                @if ($isSoldOut)
                                                ...
                                                @elseif ($sold > 0)
                                                    <div class="badge bg-success">Selling</div>
                                                @else
                                                    <div class="badge bg-warning">No Sales</div>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="2" class="text-center text-muted">No upcoming events</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
            ...
            <div class="col-sm-12 col-md-12 col-lg-6 col-xl-4">
                <div class="card overflow-hidden">
                    <div class="card-header pb-0">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-10 mt-2">Sales Summary</h4>
                            <i class="mdi mdi-dots-horizontal text-gray"></i>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="pt-3">
                            <div class="row">
                                <div class="col-sm-8">
                                    <h6 class="mb-0 tx-15 d-flex"><span class="legend bg-primary-gradient brround"></span>Processing</h6>
                                    <p class="text-muted tx-13 mb-0">{{ $process_count ?? 0 }} orders</p>
                                </div>
                            </div>
                        </div>
                        <div class="pt-3">
                            <div class="row">
                                <div class="col-sm-8">
                                    <h6 class="mb-0 tx-15 d-flex"><span class="legend bg-success-gradient brround"></span>Completed</h6>
                                    <p class="text-muted tx-13 mb-0">{{ $invoice_complete_count ?? 0 }} orders</p>
                                </div>
                            </div>
                        </div>
                        <div class="pt-3">
                            <div class="row">
                                <div class="col-sm-8">
                                    <h6 class="mb-0 tx-15 d-flex"><span class="legend bg-danger-gradient brround"></span>On Hold</h6>
                                    <p class="text-muted tx-13 mb-0">{{ $outside_hold_count ?? 0 }} tickets</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-12 col-xl-8 col-md-12 col-sm-12">
                <div class="card card-dashboard-events">
                    <div class="card-header pb-0">
                        ...
                        <p class="text-muted tx-12 mb-0">Preview of upcoming events and ticket availability.</p>
                    </div>
                    <div class="card-body">
                        <div class="list-group">
                            @forelse ($upcoming_events ?? [] as $upcoming_event)
                                @php
                                    ...
                                    $isSoldOut = $total > 0 && $sold >= $total;
                                    $indicatorClass = $isSoldOut ? 'bg-danger-gradient' : ($sold > 0 ? 'bg-info-gradient' : 'bg-primary-gradient');
                                @endphp
                                <div class="list-group-item {{ $loop->last ? 'border-bottom-0' : '' }}">
                                    <div class="event-indicator {{ $indicatorClass }}"></div>
                                    <label>{{ \Carbon\Carbon::parse($upcoming_event->event_from_date)->format('M d') }} <span>{{ \Carbon\Carbon::parse($upcoming_event->event_from_date)->format('l') }}</span></label>
                                    <h6>{{ $upcoming_event->event_name }}</h6>
                                    <p>{{ $upcoming_event->venue_name }}, {{ $upcoming_event->city_name }}, {{ $upcoming_event->country_name }}</p>
                                    <small>
                                        @if ($isSoldOut)
                                        ...
                                        @endif
                                        ({{ $sold }} sold{{ $total > 0 ? ' / ' . $total : '' }})
                                    </small>
                                    <div class="mt-2">
                                        <a href="{{ url('tickets/manage_tickets', $upcoming_event->id) }}" class="btn btn-sm btn-primary">Manage Tickets</a>
                                    </div>
                                </div>
                            @empty
                                <div class="list-group-item border-top-0">
                                    <p class="text-center text-muted mb-0">No upcoming events found.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>

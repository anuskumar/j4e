<?php $page = 'venuetype/update'; ?>
@extends('admin.layout.app')

@section('page_title', 'Edit Venue Type')

@section('breadcrumbs')
    <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ url('venuetype/list') }}">Venue Types</a></li>
    <li class="breadcrumb-item active" aria-current="page">Edit Venue Type</li>
@endsection
    ...
    <div class="col-lg-4">
        <div class="card mg-b-20">
            <div class="card-body text-center">
                ...
                <h5 class="main-profile-name mb-1" id="preview-name">{{ $data->venue_type_name ?? 'Venue Type' }}</h5>
                ...
                <div class="main-profile-contact-list">
                    <div class="media">
                        ...
                        <div class="media-body">
                            <span>Type Name</span>
                            <div id="preview-type-name">{{ $data->venue_type_name ?? 'Not set yet' }}</div>
                        </div>
                    </div>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body">
                ...
                <form class="form-horizontal" action="{{ url('venuetype/update') }}" method="POST" id="venuetype-edit-form">
                    @csrf
                    ...
                    <div class="form-group form-section-spacer">
                        <label class="form-field-label" for="venue_type_name">Venue Type Name <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="fe fe-tag"></i></span>
                            <input type="text"
                                class="form-control @error('venue_type_name') is-invalid @enderror"
                                name="venue_type_name"
                                id="venue_type_name"
                                placeholder="Enter venue type name"
                                value="{{ old('venue_type_name', $data->venue_type_name) }}"
                                maxlength="255"
                                required>
                        </div>
                        @error('venue_type_name')
            ...
            <div class="card-footer d-flex justify-content-between">
                ...
                <button type="submit" form="venuetype-edit-form" class="btn btn-primary waves-effect waves-light">
                    <i class="fe fe-save me-1"></i> Update Venue Type
                </button>
            </div>
        </div>
    </div>
...
jQuery(document).ready(function ($) {
    function updatePreview() {
        const name = $('#venue_type_name').val().trim();
        const isActive = $('#is_active_switch').is(':checked');

        $('#preview-name').text(name || 'Venue Type');
    ...
    $('#venue_type_name').on('input', updatePreview);
});
</script>
@endpush

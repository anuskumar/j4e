...
@php
    $currentImageUrl = $data->event_image
        ? asset('storage/uploads/events/' . $data->event_image)
        : asset('assets/img/events/event-01.jpg');
    $selectedArtists = $data->artists ? json_decode($data->artists, true) : [];
    $selectedTicketTypes = $data->ticket_types ? json_decode($data->ticket_types, true) : [];
    $startTime = old('event_start_time', $eventTiming && $eventTiming->from_time ? \Carbon\Carbon::parse($eventTiming->from_time)->format('H:i') : '');
    $endTime = old('event_end_time', $eventTiming && $eventTiming->to_time ? \Carbon\Carbon::parse($eventTiming->to_time)->format('H:i') : '');
@endphp
    ...
    <div class="col-lg-4">
        <div class="card mg-b-20">
            <div class="card-body text-center">
                ...
                <h5 class="main-profile-name mb-1" id="preview-event-name">{{ $data->event_name }}</h5>
                <p class="main-profile-name-text text-muted mb-2">Priority: <span id="preview-priority">{{ old('priority', $data->priority ?? 0) }}</span></p>
                <p class="form-field-hint mb-0" id="event-image-file-name">
                    @if ($data->event_image)
                        Current image uploaded — replace optional
                    @else
                ...
                <div class="main-profile-contact-list">
                    ...
                    <div class="media mb-0">
                        ...
                        <div class="media-body">
                            ...
                            <div id="preview-fee">{{ old('seller_fee_percent', $data->seller_fee_percent ?? 10) }}%</div>
                        </div>
                    </div>
                    <div class="media mb-0 mt-3">
                        ...
                        <div class="media-body">
                            ...
                            <div id="preview-customer-fee">{{ old('customer_fee_percent', $data->customer_fee_percent ?? 0) }}%</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body">
                ...
                <form class="form-horizontal" action="{{ url('events/update') }}" method="POST" id="event-edit-form" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $data->id }}">
                    <input type="file" name="event_image" id="event_image" class="d-none" accept="image/jpeg,image/png,image/jpg,image/webp" @if(!$data->event_image) required @endif>
                    @error('event_image')
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="event_tag" id="event_tag" class="form-control select2-single master-data-select @error('event_tag') is-invalid @enderror" data-create-modal="#quickCreateEventTagModal" required>
                                ...
                                @foreach ($eventTags as $val)
                                    <option value="{{ $val->id }}" {{ old('event_tag', $data->event_tag) == $val->id ? 'selected' : '' }}>
                                        {{ $val->tag_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="event_type" id="event_type" class="form-control select2-single master-data-select @error('event_type') is-invalid @enderror" data-create-modal="#quickCreateEventTypeModal" required>
                                ...
                                @foreach ($event_type as $type)
                                    <option value="{{ $type->id }}" {{ old('event_type', $data->event_type) == $type->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number"
                                    ...
                                    value="{{ old('seller_fee_percent', $data->seller_fee_percent ?? 10) }}"
                                    required>
                            </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="artists[]" id="artists" multiple class="form-control select2-multiple master-data-select" data-create-modal="#quickCreateArtistModal" data-placeholder="Select artists">
                                @foreach ($artists as $art)
                                    <option value="{{ $art->id }}"
                                        {{ (is_array(old('artists', $selectedArtists)) && in_array($art->id, old('artists', $selectedArtists))) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="date"
                                    ...
                                    id="event_from_date"
                                    value="{{ old('event_from_date', $data->event_from_date) }}"
                                    required>
                            </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="date"
                                    ...
                                    name="event_to_date"
                                    id="event_to_date"
                                    value="{{ old('event_to_date', $data->event_to_date) }}"
                                    required>
                            </div>
                            @error('event_to_date')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <div class="d-flex align-items-center justify-content-between border rounded px-3" style="min-height: 38px;">
                                ...
                                <div class="form-check form-switch mb-0">
                                    <input class="form-check-input"
                                        ...
                                        {{ old('event_is_active', $data->event_is_active) == '1' ? 'checked' : '' }}>
                                    <input type="hidden" name="event_is_active" id="event_is_active" value="{{ old('event_is_active', $data->event_is_active) }}">
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number"
                                    ...
                                    value="{{ old('customer_fee_percent', $data->customer_fee_percent ?? 0) }}"
                                    required>
                            </div>
                    ...
                    <div class="form-group mb-0">
                        ...
                        <div class="input-group">
                            ...
                            <textarea class="form-control @error('event_desc') is-invalid @enderror"
                                ...
                                id="event_desc"
                                rows="4"
                                placeholder="Enter event description">{{ old('event_desc', $data->event_desc) }}</textarea>
                        </div>
                        @error('event_desc')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
            ...
            <div class="card-footer d-flex justify-content-between">
                <a href="{{ url('events/view', $data->id) }}" class="btn btn-outline-secondary">Cancel</a>
                <button type="submit" form="event-edit-form" class="btn btn-primary waves-effect waves-light">
                    <i class="fe fe-save me-1"></i> Update Event
                </button>
            </div>
        </div>
    </div>
...
jQuery(document).ready(function ($) {
    const defaultImage = @json($currentImageUrl);
    const fallbackImage = @json(asset('assets/img/default-event.jpg'));
    const hasExistingImage = @json((bool) $data->event_image);
    ...
    $('.master-data-select').on('select2:select', function (event) {
        if (event.params.data.id !== CREATE_VALUE) return;
        ...
        if (modalSelector) openQuickCreateModal(modalSelector);
    ...
    function showFormErrors($form, errors) {
        $.each(errors, function (field, messages) {
            ...
            if ($error.length) $error.text(messages[0]);
        });
    }
    ...
    function insertSelectOption($select, item, selectValue) {
        ...
        if (item.label) $option.attr('data-label', item.label);
        $select.find('option[value="' + CREATE_VALUE + '"]').before($option);

        if (selectValue) {
            if (isMultiple) {
                const currentValues = $select.val() || [];
                if (currentValues.indexOf(String(item.id)) === -1) currentValues.push(String(item.id));
    ...
    function submitQuickCreate($form, url, $select, options) {
        ...
        $.ajax({
            ...
            const message = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Unable to save. Please try again.';
    ...
    $('#quick-create-event-type-form').on('submit', function (event) {
        event.preventDefault();
        submitQuickCreate($(this), quickCreateUrls.event_type, $('#event_type'), { onSuccess: function () { updatePreview(); } });
    });
    ...
    function updatePreview() {
        ...
        if (fromDate && toDate) datePreview = formatDate(fromDate) + ' — ' + formatDate(toDate);
        else if (fromDate) datePreview = 'From ' + formatDate(fromDate);
        if (startTime && endTime) datePreview += (datePreview !== 'Not set yet' ? ' · ' : '') + startTime + ' — ' + endTime;
        else if (startTime) datePreview += (datePreview !== 'Not set yet' ? ' · ' : '') + 'From ' + startTime;
    ...
    function handleImageFile(file) {
        ...
        if (file.size > 4 * 1024 * 1024) {
            ...
            $('#event-image-file-name').text(hasExistingImage ? 'Current image uploaded — replace optional' : 'JPG, PNG or WEBP — recommended 1500×700px');
            return;
        }
    ...
    $('#event-edit-form').on('submit', function (e) {
        if (!hasExistingImage && !$('#event_image')[0].files.length) {
            e.preventDefault();
